/**
 * Seed homepage testimonials (text + video) for SRI KUMARAN SILKS.
...
import { shopMediaBaseUrl } from "./lib/supabase-project.mjs";
...
const mediaRows: MediaRow[] = TESTIMONIAL_IMAGES.map((imageUrl, index) => ({
  ...
  key: imageUrl,
  alt: `SRI KUMARAN SILKS customer — Tamil Nadu saree ${index + 1}`,
}));
...
const testimonialRows: TestimonialRow[] = [
  ...
  },
  {
    ...
    location: "Salem, Tamil Nadu",
    quote:
      "Festival saree pick from SRI KUMARAN SILKS — loved the drape and border finish.",
