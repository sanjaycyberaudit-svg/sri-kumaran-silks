...
function CustomerAvatar({
  ...
  return (
    <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-primary to-[#A82020] text-sm font-bold text-white ring-2 ring-primary/15 ring-offset-2 ring-offset-background">
      {initials || "ST"}
    </div>
  );
}
    ...
    <article className="flex w-full flex-col rounded-2xl border border-primary/12 bg-card p-4 shadow-[0_16px_40px_-28px_rgba(107,24,88,0.45)] sm:min-h-[280px] sm:p-6">
      ...
      {node.quote ? (
        ...
        <p className="flex-1 text-sm text-muted-foreground">
          Thank you for shopping with SRI KUMARAN SILKS.
        </p>
      )}
...
function ModernVideoTestimonialCard({ node }: { node: TestimonialNode }) {
  ...
  return (
    <article className="group flex w-full flex-col overflow-hidden rounded-2xl border border-primary/15 bg-[#8B1818]/5 shadow-[0_16px_40px_-24px_rgba(200,40,40,0.5)]">
      <div className="relative aspect-[9/13] w-full max-h-[min(62vh,360px)] bg-muted sm:aspect-[3/4] sm:max-h-none">
        ...
        />
        <div className="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#8B1818]/95 via-[#8B1818]/50 to-transparent p-3 pt-12 sm:p-4 sm:pt-16">
          <StarRating rating={node.rating ?? 5} />
          {node.quote ? (
