...
/** Sri Kumaran Silks saree categories (homepage carousel + /collections/[slug]) */
export const KUMARAN_COLLECTION_LABELS = [
  "Softie Sarees",
...
function collectionCopy(label: string) {
  return {
    title: label,
    description: `Explore our ${label} at ${brandTitleName} — premium sarees for every occasion.`,
  };
}

export type SeedKumaranCollectionsOptions = {
  /** Remove old demo furniture collections (Bathroom, Kitchen, etc.) */
  removeDemo?: boolean;
};

export default async function seedKumaranCollections(
  options: SeedKumaranCollectionsOptions = {},
) {
  ...
  for (let i = 0; i < KUMARAN_COLLECTION_LABELS.length; i++) {
    ...
    const imageAlt = `${label} — Tamil Nadu saree model, ${brandTitleName}`;
  ...
  console.log(
    `\nDone. ${created} created, ${updated} updated (${KUMARAN_COLLECTION_LABELS.length} categories total).`,
  );
}
